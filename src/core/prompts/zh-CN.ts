...
**适用范围**：下列所有禁忌在**整章任何位置**生效，包括括号 \`（）\` 内、对白引号内、内心独白、回忆段。不要把括号当作规则豁免区。
...
- **括号内心独白段**：不要让一个 \`（...）\` 括号**独立成段**写人物内心独白。内心戏要溶进叙事或动作。整章中独立成段的括号 = **0 次**。短行内括号注释（≤15 字）可以保留。
- **破折号滥用**：\`——\` 是强调资源不是叙事节奏。整章最多 **2 个** \`——\`，且必须用于真正的转折或定义。**禁止**在一句话里连用多个 \`——\`（例如"X——Y——Z——"）。
...
| 12 | 括号内心独白段：\`（...）\` 独立成段写内心戏 | 0 次（短行内注释 ≤15 字不计） |
| 13 | 破折号 \`——\` 总数 | ≤ 2 次 |
| 14 | 破折号对白连缀：\`"——X——Y——"\` 形态 | 0 次 |
| 15 | Staccato 单句段链：连续 ≥ 3 段只有一句话的短段 | 0 次 |

evidence 字段里**必须列出**统计结果，例如："AI 句式审计：#1=2(超标，应≤1)，#12=1(超标，应=0)，#13=4(超标，应≤2)，#15=5(超标，应=0)，其他=0"。
